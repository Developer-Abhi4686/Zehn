...
import { FileText, Send, Copy, RotateCcw } from 'lucide-react';
import { getGeminiResponse, prompts } from '../../lib/geminiService';
import ReactMarkdown from 'react-markdown';
import SaveButton from '../../components/SaveButton';

export default function TestPaperGenerator({ userClass }: { userClass: string | null }) {
  ...
  const handleGenerate = async () => {
    if (!desc) return;
    setLoading(true);
    const res = await getGeminiResponse(`Generate a full test paper based on: ${desc}`, prompts.testPaperGenerator, userClass);
    setResponse(res);
    setLoading(false);
  };
